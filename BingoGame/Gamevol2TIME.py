...
def main(stdscr, xaxis, yaxis, words, mq, maxplayer, playernumber, roundfile):
    ...
    while True:
        # Überprüfung auf Nachrichten
        message = check_for_message(mq)
        if message:  # Nachricht vorhanden
            ...
            stdscr.refresh()
            nichtverloren = False
            break  # Beenden der Schleife, wenn eine Nachricht empfangen wurde

        # Nicht blockierende Überprüfung auf Tastatureingaben
        key = stdscr.getch()

        if key == ord('x'):
            break

        if nichtverloren and key == curses.KEY_MOUSE:  # Überprüft, ob das Ereignis key ein Mausereignis ist
        ...
        # Pause für eine Sekunde, um im Sekundentakt zu bleiben
        time.sleep(1)
